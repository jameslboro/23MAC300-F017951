% James Jarman (j.jarman-20@lboro.ac.uk)
% Loughborough University
% 2024

% NOTE:
% basisfunctionsP1P2.m;       basisfunctionsP2P1.m;
%             mat0X       =       matX0'
%             mat0Y       =       matY0'
%             matX0       =       mat0X'
%             matY0       =       mat0Y'

clear; close all;

%% P1 functions psi_i(x,y) N=3

% Basis functions expressed as:
% psi(xi,yi) = c1*xi + c2*yi + c3
% Numbering of the nodes:
% 1 (0,0)
% 2 (1,0)
% 3 (0,1)

xi = [0 1 0];
yi = [0 0 1];

A1 = sym(zeros(3,3));

for i = 1:3
    A1(i,:) = [xi(i), yi(i), 1];
end

b1 = sym(eye(3,3));

% Coefficients of the basis functions
c1 = linsolve(A1,b1);


syms x y

psi = sym(zeros(3,1)); % expression of the basis function psi

dpsidx = sym(zeros(3,1));
dpsidy = sym(zeros(3,1));
for j=1:3
    psi(j,1) = c1(1,j)*x + ...
               c1(2,j)*y + ...
               c1(3,j);
    dpsidx(j,1) = diff(psi(j,1),x);
    dpsidy(j,1) = diff(psi(j,1),y);
end

%% P2 functions phi_j(x,y) M=6

% Basis functions expressed as:
% phi(xi,yi) = c1*xi^2 + c2*yi^2 + c3*xi*yi + c4*xi + c5*yi + c6
% Numbering of the nodes:
% 1 (0,0)
% 2 (1,0)
% 3 (0,1)
% 4 (1/2,0)
% 5 (1/2,1/2)
% 6 (0,1/2)

xi = [0 1 0 1/2 1/2 0];
yi = [0 0 1 0 1/2 1/2];

A2 = sym(zeros(6,6));

for i = 1:6
    A2(i,:) = [xi(i)^2, yi(i)^2, xi(i)*yi(i), xi(i), yi(i), 1];
end

b2 = sym(eye(6,6));

% Coefficients of the basis functions
c2 = linsolve(A2,b2);


syms x y

phi = sym(zeros(6,1)); % expression of the basis function phi

dphidx = sym(zeros(6,1));
dphidy = sym(zeros(6,1));
for j = 1:6
    phi(j,1) = c2(1,j)*x*x + ...
               c2(2,j)*y*y + ...
               c2(3,j)*x*y + ...
               c2(4,j)*x + ...
               c2(5,j)*y + ...
               c2(6,j);
    dphidx(j,1) = diff(phi(j,1),x);
    dphidy(j,1) = diff(phi(j,1),y);
end

%% 

mat0X = sym(zeros(length(phi),length(psi)));
mat0Y = sym(zeros(length(phi),length(psi)));
matX0 = sym(zeros(length(phi),length(psi)));
matY0 = sym(zeros(length(phi),length(psi)));

for i = 1:length(phi)
    for j = 1:length(psi)
        mat0X(i,j) = int(int(phi(i,1)*dpsidx(j,1),y,0,-x+1),x,0,1);
        mat0Y(i,j) = int(int(phi(i,1)*dpsidy(j,1),y,0,-x+1),x,0,1);
        matX0(i,j) = int(int(dphidx(i,1)*psi(j,1),y,0,-x+1),x,0,1);
        matY0(i,j) = int(int(dphidy(i,1)*psi(j,1),y,0,-x+1),x,0,1);
    end
end
