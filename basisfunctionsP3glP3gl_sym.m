% James Jarman (j.jarman-20@lboro.ac.uk)
% Loughborough University
% 2024

clear; close all;

%% P3gl functions phi_i(x,y) N=10

% Basis functions expressed as:
% phi(xi,yi) = c1*xi^3 + c2*yi^3 + c3*xi^2*yi + c4*xi*yi^2 + ...
%              c5*xi^2 + c6*yi^2 + c7*xi*yi + c8*xi + c9*yi + c10
% Numbering of the nodes:
%  1 (0,0)
%  2 (1,0)
%  3 (0,1)
%  4 ((5-sqrt(5))/10,0)
%  5 ((5+sqrt(5))/10,0)
%  6 (1-((5-sqrt(5))/10),5-sqrt(5)/10)
%  7 (1-((5+sqrt(5))/10),5+sqrt(5)/10)
%  8 (0,(5+sqrt(5))/10)
%  9 (0,(5-sqrt(5))/10)
% 10 (1/3,1/3)

sqrt5 = sym(sqrt(5)); % create symbolic variable for sqrt(5)

xi = [0, 1, 0, (sqrt5-1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5-1)/(2*sqrt5), 0, 0, 1/3];
yi = [0, 0, 1, 0, 0, (sqrt5-1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5-1)/(2*sqrt5), 1/3];

A1 = sym(zeros(10,10));

for i = 1:10
    A1(i,:) = [xi(i)^3, yi(i)^3, xi(i)^2*yi(i), xi(i)*yi(i)^2, xi(i)^2, yi(i)^2, xi(i)*yi(i), xi(i), yi(i), 1];
end

b1 = sym(eye(10,10));

% Coefficients of the basis functions
c1 = linsolve(A1,b1);


syms x y

phi = sym(zeros(10,1)); % expression of the basis function phi

dphidx = sym(zeros(10,1));
dphidy = sym(zeros(10,1));
for j = 1:10
    phi(j,1) = c1(1,j)*x^3 + ...
               c1(2,j)*y^3 + ...
               c1(3,j)*x^2*y + ...
               c1(4,j)*x*y^2 + ...
               c1(5,j)*x^2 + ...
               c1(6,j)*y^2 + ...
               c1(7,j)*x*y + ...
               c1(8,j)*x + ...
               c1(9,j)*y + ...
               c1(10,j);
    dphidx(j,1) = diff(phi(j,1),x);
    dphidy(j,1) = diff(phi(j,1),y);
end

%% P3gl functions psi_j(x,y) M=10

% NOTE:
% This 2nd section is not necessary for cases P1P1, P2P2, P3P3.
% The final matrices can be computed using only the first section as
% phi = psi, hence dphidx = dpsidx and dphidy = dpsidy.
% As a result we also find: mat0X = matX0' and mat0Y = matY0'.

% This 2nd section has been included for consistency with all other
% combinations of basisfunctionsP#P#.m codes.

% Basis functions expressed as:
% psi(xi,yi) = c1*xi^3 + c2*yi^3 + c3*xi^2*yi + c4*xi*yi^2 + ...
%              c5*xi^2 + c6*yi^2 + c7*xi*yi + c8*xi + c9*yi + c10
% Numbering of the nodes:
%  1 (0,0)
%  2 (1,0)
%  3 (0,1)
%  4 ((5-sqrt(5))/10,0)
%  5 ((5+sqrt(5))/10,0)
%  6 (1-((5-sqrt(5))/10),5-sqrt(5)/10)
%  7 (1-((5+sqrt(5))/10),5+sqrt(5)/10)
%  8 (0,(5+sqrt(5))/10)
%  9 (0,(5-sqrt(5))/10)
% 10 (1/3,1/3)

sqrt5 = sym(sqrt(5)); % create symbolic variable for sqrt(5)

xi = [0, 1, 0, (sqrt5-1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5-1)/(2*sqrt5), 0, 0, 1/3];
yi = [0, 0, 1, 0, 0, (sqrt5-1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5+1)/(2*sqrt5), (sqrt5-1)/(2*sqrt5), 1/3];

A2 = sym(zeros(10,10));

for i = 1:10
    A2(i,:) = [xi(i)^3, yi(i)^3, xi(i)^2*yi(i), xi(i)*yi(i)^2, xi(i)^2, yi(i)^2, xi(i)*yi(i), xi(i), yi(i), 1];
end

b2 = sym(eye(10,10));

% Coefficients of the basis functions
c2 = linsolve(A2,b2);


syms x y

psi = sym(zeros(10,1)); % expression of the basis function psi

dpsidx = sym(zeros(10,1));
dpsidy = sym(zeros(10,1));
for j = 1:10
    psi(j,1) = c2(1,j)*x^3 + ...
               c2(2,j)*y^3 + ...
               c2(3,j)*x^2*y + ...
               c2(4,j)*x*y^2 + ...
               c2(5,j)*x^2 + ...
               c2(6,j)*y^2 + ...
               c2(7,j)*x*y + ...
               c2(8,j)*x + ...
               c2(9,j)*y + ...
               c2(10,j);
    dpsidx(j,1) = diff(psi(j,1),x);
    dpsidy(j,1) = diff(psi(j,1),y);
end

%% 

mat0X = sym(zeros(10,10));
mat0Y = sym(zeros(10,10));
matX0 = sym(zeros(10,10));
matY0 = sym(zeros(10,10));

for i = 1:10
    for j = 1:10
        mat0X(i,j) = int(int(phi(i,1)*dpsidx(j,1),y,0,-x+1),x,0,1);
        mat0Y(i,j) = int(int(phi(i,1)*dpsidy(j,1),y,0,-x+1),x,0,1);
        matX0(i,j) = int(int(dphidx(i,1)*psi(j,1),y,0,-x+1),x,0,1);
        matY0(i,j) = int(int(dphidy(i,1)*psi(j,1),y,0,-x+1),x,0,1);
    end
end
